...
class Window:
    ...
    def draw_object(self, object: Obj3D.Objeto3D):
        obj_coords = object.calculate_perspective_normalized_coords(self.__transformator.dop,
                                                                    self.__transformator.matrix)
        ...
        if object.obj_type == Obj3D.ObjectType.POINT:
            ...
            self.draw_curve(obj_coords, object.color)
    ...
    def draw_bicurve(self, coords: list[tuple[float]], color: str) -> None:
        for curve in coords:
